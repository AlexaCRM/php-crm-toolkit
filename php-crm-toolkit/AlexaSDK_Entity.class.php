...
/**
 * This class used for work with Dynamics CRM entites
 */
class AlexaSDK_Entity extends AlexaSDK_Abstract {
		 ...
		 * @var Array
		 */
		private $manyToManyRelationships = Array();
		 ...
		 * @var Array
		 */
		private $manyToOneRelationships = Array();
		 ...
		 * @var Array
		 */
		private $oneToManyRelationships = Array();
		 ...
		 */
		function __construct(AlexaSDK $_auth, $_logicalName = NULL, $_ID = NULL, $fieldset = NULL) {
			try{
				...
				if ($this->auth->isEntityDefinitionCached($this->entityLogicalName)) {
					...
					$isDefined = $this->auth->getCachedEntityDefinition($this->entityLogicalName, $this->entityData, $this->properties, $this->propertyValues, $this->mandatories, $this->optionSets, $this->displayName, $this->entitytypecode, $this->entityDisplayName, $this->entityDisplayCollectionName, $this->entityDescription, $this->manyToManyRelationships, $this->manyToOneRelationships, $this->oneToManyRelationships);

					if ($isDefined) {
						AlexaSDK_Logger::log("Cached entity definition: " . $this->entityLogicalName);
						/* Set EntityValues if specified Entity ID */
						if ($_ID != NULL) {
							/* Set the ID of Entity record */
							$this->setID($_ID);
							/* Get the raw XML data */
							$rawSoapResponse = $this->auth->retrieveRaw($this, $fieldset);
				...
				foreach ($this->entityData->Attributes[0]->AttributeMetadata as $attribute) {
					...
					if (!empty($attribute->OptionSet) && !empty($attribute->OptionSet->Name)) {
						...
						AlexaSDK_Logger::log( 'Attribute '.(String)$attribute->SchemaName.' is an OptionSet');
						AlexaSDK_Logger::log( "\tName:\t".$optionSetName.($optionSetGlobal ? ' (Global)' : ''));
						AlexaSDK_Logger::log( "\tType:\t".$optionSetType);
						...
						switch ($optionSetType) {
							...
							case 'State':
								foreach ($attribute->OptionSet->Options->OptionMetadata as $option) {
									...
									if (array_key_exists($value, $optionSetValues)) {
										trigger_error('Option ' . $label . ' of OptionSet ' . $optionSetName . ' used by field ' . (String) $attribute->SchemaName . ' has the same Value as another Option in this Set', E_USER_WARNING);
									} else {
										/* Store the Option */
										$optionSetValues[$value] = $label;
									}
							...
							default:
								...
								trigger_error('No OptionSet handling implemented for Type ' . $optionSetType . ' used by field ' . (String) $attribute->SchemaName . ' in Entity ' . $this->entityLogicalName, E_USER_WARNING);
						}
						/* DebugLogging - Identify the OptionSet Values */
						foreach ($optionSetValues as $value => $label) {
							AlexaSDK_Logger::log("\t\tOption ".$value.' => '.$label);
						}
						...
						if (array_key_exists($optionSetName, $this->optionSets)) {
							...
							if (!$optionSetGlobal) {
								/* 
								 * trigger_error('OptionSet ' . $optionSetName . ' used by field ' . (String) $attribute->SchemaName . ' has a name clash with another OptionSet in Entity ' . $this->entityLogicalName, E_USER_WARNING);
								 */
							}
						} else {
					...
					//AlexaSDK_Logger::log(					
						/*
						  ...
						  echo "</pre>"; */
					/* Define empty field values for entity */
				...
				/*
				 * ToDo: add definitions for other relationship types
				 */

				/* Ensure that this Entity Definition is Cached for next time */
				$this->auth->setCachedEntityDefinition($this->entityLogicalName, $this->entityData, $this->properties, $this->propertyValues, $this->mandatories, $this->optionSets, $this->displayName, $this->entitytypecode, $this->entityDisplayName, $this->entityDisplayCollectionName, $this->entityDescription, $this->manyToManyRelationships, $this->manyToOneRelationships, $this->oneToManyRelationships);
				/* Set EntityValues if specified Entity ID */
				if ($_ID != NULL) {
					/* Set the ID of Entity record */
					$this->setID($_ID);
		...
		public function __get($property) {
			try{
				AlexaSDK_Logger::log("__get() entity property: ".$this->entityLogicalName." > ".$property);
		...
		public function __set($property, $value) {
			try{
				//AlexaSDK_Logger::log("__set() entity property: ".$this->entityLogicalName." > ".$property."; value:".$value);
				/* Handle special fields */
				switch (strtoupper($property)) {
					case 'ID':
						$this->setID($value);
			...
			switch ($this->properties[$property]["Type"]) {
					...
					switch ($this->properties[$property]["Format"]) {
						case "Text":
							if ($value && !$this->validator->validateItem($value, 'anything')) {
								$this->errors[$property] = "Invalid text value";
							}
							break;
						case "Email":
							if ($value && !$this->validator->validateItem($value, 'email')) {
								$this->errors[$property] = "Invalid email";
							}
							break;
						default:
				...
				case "Integer":
					if ($value && !$this->validator->validateItem($value, 'amount')) {
						$this->errors[$property] = "Invalid number value";
					}
					break;
				case "Double":
					if ($value && !$this->validator->validateItem($value, 'float')) {
						$this->errors[$property] = "Invalid number value. Enter a decimal number without currency symbol or thousands separator.";
					}
					break;
				case "Money":
					if ($value && !$this->validator->validateItem($value, 'float')) {
						$this->errors[$property] = "Invalid money value. Enter a decimal number without currency symbol or thousands separator.";
					}
					break;
				case "Memo":
					break;
		 ...
		 */
		public static function fromDOM(AlexaSDK $auth, $entityLogicalName, DOMElement $domNode, $queryXML = NULL) {
			try{
				...
				//AlexaSDK_Logger::log('Entity <'.$entity->ID.'> has EntityState: '.$entityState);
				/* Parse the Attributes & FormattedValues to set the properties of the Entity */
				$entity->setAttributesFromDOM($auth, $attributesNode, $formattedValuesNode, $queryXML);
		 ...
		 * @ignore
		 */
		private function setAttributesFromDOM(AlexaSDK $auth, DOMElement $attributesNode, DOMElement $formattedValuesNode, $queryXML = NULL) {
			try{
							...
							if (strpos($attributeKey, '.') === FALSE) {
								...
								if (array_key_exists($aliasName, $this->propertyValues)) {
									...
									if ($queryXML){
										...
										$linkedEntityElement = NULL;
										foreach($queryDom->getElementsByTagName('link-entity') as $linkedEntityNode){

												if ($linkedEntityNode->hasAttribute("alias") && $linkedEntityNode->getAttribute("alias") == $aliasName){
														$linkedEntityElement = $linkedEntityNode;
												}
										}

										$propertyField = $linkedEntityElement->getAttribute("to");
										$propertyValue = $keyValueNode->getElementsByTagName('value')->item(0)->getElementsByTagName('Value')->item(0)->textContent;
									}

									if (isset($propertyField) && isset($this->propertyValues[$propertyField]["Value"])){
										$storedValue = $this->propertyValues[$propertyField]["Value"];
									}else{
										/* Create a new Entity of the appropriate type */
										$storedValue = self::fromLogicalName($auth, $aliasEntityName);
									}
								...
								$storedValue->setAttributesFromDOM($auth, $aliasAttributesNode, $aliasFormattedValuesNode, $queryXML);
								/* Finally, ensure that this is stored on the Entity using the Alias */
								$attributeKey = ($propertyField) ? $propertyField : $aliasName;
							}
							break;
						default:
		...
		public function printDetails($recursive = false, $tabLevel = 0, $printEmpty = true) {
			...
			foreach ($propertyList as $property) {
				...
				if ($recursive && $propertyDetails['AttributeOf'] != NULL)
					continue;
				/* Don't print NULL fields if printEmpty is FALSE */
				if (!$printEmpty && $this->propertyValues[$property]['Value'] == NULL)
					continue;
				...
				switch (strtolower($propertyDetails['Type'])) {
					...
					default:
						...
						trigger_error('No output handling implemented for type ' . $propertyDetails['Type'] . ' used by field ' . $property, E_USER_WARNING);
