...
class OnlineFederation extends Authentication {
     ...
     * @ignore
     */
    public function getSecurityHeaderNode( Array $securityToken ) {
    ...
    protected function requestSecurityToken( $securityServerURI, $loginEndpoint, $loginUsername, $loginPassword ) {
        try {
            ...
            $this->client->logger->info( 'Got OnlineFederation security token - expires at: ' . date( 'r', $securityToken['expiryTime'] ) );

            /* Return an associative Array */

            return $securityToken;
        } catch ( Exception $e ) {
            $this->client->logger->error( 'Caught exception while requesting OnlineFederation security token', [
                'exception' => $e,
