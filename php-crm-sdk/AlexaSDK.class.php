...
class AlexaSDK extends AlexaSDK_Abstract{
	...
	protected static $maximumRecords = self::MAX_CRM_RECORDS;
         ...
         * @return AlexaSDK
         */
        function __construct(AlexaSDK_Settings $_settings, $_debug = NULL) {
            ...
            switch ($this->settings->authMode){
                case "OnlineFederation":
                    $this->authentication = new AlexaSDK_Office365($this->settings, $this);
                    break;
                case "Federation":
        ...
        public function clearCache(){
            ...
            return $this->cacheClass->cleanup();
        }
	 ...
	 * @ignore
	 */
	public static function parseRetrieveAllEntitiesResponse1($soapResponse) {
	...
	private function getOrganizationDOM() {
		...
		if (self::$debugMode) echo 'Getting WSDL data for Organization DOM from: '.$this->settings->organizationUrl.'?wsdl'.PHP_EOL;
	 ...
	 * @ignore
	 */
        protected static function generateExecuteActionRequest($requestName, $parameters = NULL, $requestType = NULL){
	 ...
	 * Retrieves changes in metadata
        ...
        public function retrieveMetadataChanges(){
                ...
                $retrieveMetadataChangesNode = self::generateRetrieveMetadataChangesRequest();
                /* Turn this into a SOAP request, and send it */
                $retrieveMetadataChangesRequest = $this->generateSoapRequest($this->settings->organizationUrl, $this->getOrganizationExecuteAction(), $securityToken, $retrieveMetadataChangesNode);
	 ...
	 */
        ...
        public function importSolution($customizationFile, $overwriteUnmanagedCustomizations, $publishWorkflows){
                    
                if (!base64_decode($customizationFile)){
                        $customizationFile = base64_encode($customizationFile);
                }
            
                $parameters = array(
                    array('key' => 'OverwriteUnmanagedCustomizations',
                          'value' => $overwriteUnmanagedCustomizations,
                          'type' => 'boolean'
                        ),
                    array('key' => 'PublishWorkflows',
                          'value' => $publishWorkflows,
                          'type' => 'boolean'
                        ),
                    array('key' => 'CustomizationFile',
                          'value' => $customizationFile,
                          'type' => 'base64Binary'
                        ),
                ...
                $securityToken = $this->authentication->getOrganizationSecurityToken();
                /* Generate the XML for the Body of a Execute Action request */
                $executeActionNode = self::generateExecuteActionRequest('ImportSolution', $parameters, 'ImportSolutionRequest');

                if (self::$debugMode) echo PHP_EOL.'ExecuteAction Request: '.PHP_EOL.$executeActionNode->C14N().PHP_EOL.PHP_EOL;

                /* Turn this into a SOAP request, and send it */
                $executeActionRequest = $this->generateSoapRequest($this->settings->organizationUrl, $this->getOrganizationExecuteAction(), $securityToken, $executeActionNode);
                $soapResponse = self::getSoapResponse($this->settings->organizationUrl, $executeActionRequest, true);
        ...
        public function setCachedSecurityToken($service, $token){
                if ($this->cacheClass){
                    $this->cacheClass->set(strtolower($service)."securitytoken", serialize($token), floor($token['expiryTime'] - time()));
                }
        }
        
        public function getCachedSecurityToken($service, &$securityToken){
                if ($this->cacheClass){
                    $token = $this->cacheClass->get(strtolower($service)."securitytoken");
                    if ($token != NULL){
